...
/** A class-based builder trait that packs a [[Spore]] with a closure of type
  ...
  * val mySpore: Spore[Int => String] = new MyBuilder[Int]().pack()
  *   }}}
  *
  * @tparam T
...
trait SporeClassBuilder[+T](private[spores] val fun: T) {
  ...
  final inline def pack(): Spore[T] = {
    ${ SporeClassBuilder.packMacro('this) }
  }
}
...
private object SporeClassBuilder {
  ...
  def packMacro[T](expr: Expr[SporeClassBuilder[T]])(using Type[T], Quotes): Expr[Spore[T]] = {
